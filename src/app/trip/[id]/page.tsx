...
export default function TripDetail() {
  ...
  const initiateBooking = async (e?: React.MouseEvent) => {
    ...
    const { data: convs } = await supabase
      ...
      .select("*")
      .or(
        `and(participant1_id.eq.${currentUserId},participant2_id.eq.${trip.user_id}),and(participant1_id.eq.${trip.user_id},participant2_id.eq.${currentUserId})`,
      );
    ...
    if (convId) {
      ...
      router.push(
        `/?openChat=${convId}&partnerId=${trip.user_id}&partnerName=${encodeURIComponent(sherpaName)}&tripId=${tripId}`,
      );
    } else {
        ...
        <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-slate-100">
          <div className="bg-slate-900 p-8 text-white relative">
            <div className="flex items-center gap-4 text-2xl font-black mb-2">
              {trip.origin} <Plane className="text-orange-500" />{" "}
              {trip.destination}
            </div>
            ...
            <div className="md:col-span-2 space-y-6">
              ...
              <div className="pt-4 border-t">
                ...
                <p className="text-slate-600 text-sm leading-relaxed whitespace-pre-wrap">
                  {trip.description ||
                    "Der Sherpa hat keine zusätzliche Beschreibung angegeben."}
                </p>
              </div>
            </div>
