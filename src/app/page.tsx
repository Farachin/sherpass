...
function HomeContent() {
  ...
  const [chatMessages, setChatMessages] = useState<MessageWithRelations[]>([]);
  const [chatInput, setChatInput] = useState("");
  const [showBookingOptions, setShowBookingOptions] = useState(false);
  const [isChatLoading, setIsChatLoading] = useState(false);
  const [capacityWarnings, setCapacityWarnings] = useState<
    Record<string, { remaining: number; exceeds: boolean }>
  >({});
  ...
  const [warning, setWarning] = useState<ComplianceWarning>(null);
    ...
    if (openChatId && partnerId && partnerName && currentUserId) {
      ...
      const loadChatMessages = async (cId: string) => {
        ...
        if (data) {
          ...
          let processedData = data.map((msg) => {
            if (msg.type === "booking_request" && msg.shipment_id) {
              ...
              if (Array.isArray(msg.shipments)) {
                ...
                msg.shipments &&
                msg.shipments.id === msg.shipment_id
              ) {
              ...
              if (tripIdParam && shipment && shipment.trip_id !== tripIdParam) {
                ...
                return { ...msg, shipments: undefined };
              }
          ...
          if (tripIdParam) {
            console.log(
              "DEBUG: Filtere Chat-Nachrichten nach trip_id:",
              tripIdParam,
            );
            processedData = processedData.map((msg) => {
              ...
              if (msg.type === "booking_request" && msg.shipments) {
                ...
                if (shipment && shipment.trip_id !== tripIdParam) {
                  console.warn(
                    "DEBUG: Shipment trip_id stimmt nicht überein (zweite Prüfung):",
                    {
                      msgId: msg.id,
                      shipmentId: shipment.id,
                      shipmentTripId: shipment.trip_id,
            ...
            setChatMessages((prev) => {
              ...
              if (exists) {
                ...
                return prev
                  ...
                  .filter(
                    (m, idx, arr) =>
                      arr.findIndex((msg) => msg.id === m.id) === idx,
                  ) as MessageWithRelations[]; // Entferne Duplikate
              }
  ...
  const loadMyShipments = async (userId: string) => {
    ...
    try {
      ...
      if (error) {
        ...
        setMyShipments((data as Shipment[]) || []);
      }
    } catch (err) {
        ...
        const processedData = fallbackData.map((msg) => {
          if (msg.type === "booking_request" && msg.shipment_id) {
            ...
            if (
              contextTripId &&
              shipment &&
              shipment.trip_id !== contextTripId
            ) {
              console.warn("DEBUG: Shipment trip_id stimmt nicht überein:", {
                msgId: msg.id,
                shipmentId: shipment.id,
    ...
    if (data) {
      ...
      if (currentUserId) {
        const warnings: Record<
          string,
          { remaining: number; exceeds: boolean }
        > = {};
  ...
  const openChat = async (targetId: string, name: string, tripId?: string) => {
    ...
    const { data: convs } = await supabase
      ...
      .select("*")
      .or(
        `and(participant1_id.eq.${currentUserId},participant2_id.eq.${targetId}),and(participant1_id.eq.${targetId},participant2_id.eq.${currentUserId})`,
      );
    let cId = convs?.[0]?.id;
    if (!cId) {
  ...
  const handleBookingAccept = async (msg: MessageWithRelations) => {
    if (!msg.shipment_id || !contextTripId) return;
    await supabase
      .from("shipments")
      .update({ status: "accepted", trip_id: contextTripId })
      .eq("id", msg.shipment_id);
  ...
  const saveManifest = async () => {
    ...
    try {
      ...
      if (contextTripId) {
        ...
        console.log(
          "DEBUG: Shipment wird mit trip_id erstellt:",
          contextTripId,
        );
      } else {
        console.warn(
          "DEBUG: Keine trip_id verfügbar - Shipment wird ohne trip_id erstellt",
        );
      }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-20 relative">
      ...
      <nav className="bg-white border-b border-slate-200 h-16 flex items-center justify-between px-2 sm:px-4 lg:px-8 overflow-x-hidden max-w-full">
        <Link
          href="/"
          className="flex items-center gap-1 sm:gap-2 cursor-pointer flex-shrink-0 min-w-0"
        >
          <Plane className="text-orange-500 flex-shrink-0" />{" "}
          <span className="font-black text-lg sm:text-xl tracking-tighter truncate">
            SHERPASS
          </span>
        </Link>
        <div className="flex gap-2 sm:gap-4 items-center flex-shrink-0">
          ...
          <button
            ...
            className="hidden sm:flex text-orange-500 font-bold text-sm items-center gap-1 hover:bg-orange-50 px-3 py-2 rounded-full transition"
          >
          ...
          <button
            onClick={() => setShowOfferModal(true)}
            className="sm:hidden bg-orange-500 text-white p-2.5 rounded-full shadow-lg hover:bg-orange-600 transition flex items-center justify-center"
          >
            <Plus size={20} />
          </button>
          {!userEmail ? (
            ...
            <Link
              href="/dashboard"
              className="bg-slate-900 text-white w-9 h-9 rounded-full flex items-center justify-center font-bold text-xs hover:bg-orange-500 transition"
            >
        ...
        <>
          ...
          <div className="bg-slate-900 text-white py-3 border-y border-slate-800 overflow-hidden">
            <div className="ticker text-sm font-mono flex whitespace-nowrap">
              <span className="mx-4 text-orange-500">●</span> Ahmad: Hamburg ➔
              Kabul <span className="mx-4 text-orange-500">●</span> Maria:
              Berlin ➔ Teheran <span className="mx-4 text-orange-500">●</span>{" "}
              Jean: Paris ➔ Bamako{" "}
              <span className="mx-4 text-orange-500">●</span> Ali: Frankfurt ➔
              Mazar-i-Sharif
              <span className="mx-4 text-orange-500">●</span> Lisa: München ➔
              Dubai <span className="mx-4 text-orange-500">●</span> Karim:
              London ➔ Islamabad <span className="mx-4 text-orange-500">●</span>{" "}
              Sarah: Köln ➔ Istanbul
              <span className="mx-4 text-orange-500">●</span> Ahmad: Hamburg ➔
              Kabul <span className="mx-4 text-orange-500">●</span> Maria:
              Berlin ➔ Teheran <span className="mx-4 text-orange-500">●</span>{" "}
          ...
          <div className="max-w-7xl mx-auto px-4 py-20 text-center bg-white">
            <h2 className="text-3xl font-black text-slate-900 mb-4">
              Warum Sherpass besser ist.
            </h2>
            <p className="text-slate-500 mb-12">
              Klassischer Versand vs. Community Power.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-lg">
                ...
                <p className="text-slate-600 mb-4 text-sm leading-relaxed">
                  DHL braucht Wochen nach Afghanistan oder Mali. Ein Sherpa
                  nimmt den Direktflug.
                </p>
                <div className="flex justify-center items-center gap-2 font-bold text-sm text-green-600">
                  ✓ Ankunft in 24-48h
                </div>
              </div>
              <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-lg">
                ...
                <p className="text-slate-600 mb-4 text-sm leading-relaxed">
                  Kein anonymes Verteilerzentrum. Persönliche Übergabe
                  (Hand-to-Hand).
                </p>
              ...
              <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-lg">
                ...
                <p className="text-slate-600 mb-4 text-sm leading-relaxed">
                  Sherpass nimmt 0% Provision. Ihr einigt euch selbst auf den
                  Preis.
                </p>
                <div className="flex justify-center items-center gap-2 font-bold text-sm text-slate-900">
                  ✓ 100% Dein Geld
                </div>
              </div>
            </div>
          ...
          <div className="bg-slate-50 py-20 border-y border-slate-200">
            <div className="max-w-4xl mx-auto px-4">
              <h2 className="text-xl font-black mb-8 flex items-center gap-2">
                <Zap className="text-orange-500" /> Beliebte Verbindungen
              </h2>
              <div className="grid md:grid-cols-3 gap-4">
                {teaserTrips.length > 0 ? (
                  teaserTrips.map((trip) => (
                    <div
                      key={trip.id}
                      onClick={() => router.push(`/trip/${trip.id}`)}
                      className="bg-white p-5 rounded-xl border border-slate-200 hover:border-orange-300 cursor-pointer flex flex-col justify-between group transition shadow-sm h-32"
                    >
                      <div className="font-bold text-slate-700 text-lg">
                        {trip.origin} <span className="text-orange-500">➔</span>{" "}
                        {trip.destination}
                      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-12 text-center md:text-left">
              <div className="flex flex-col items-center md:items-start">
                ...
                <p className="text-sm text-slate-500 leading-relaxed">
                  Nutze die Suche, um Reisende auf deiner Route zu finden. Schau
                  dir ihre Profile und Bewertungen an.
                </p>
              </div>
              <div className="flex flex-col items-center md:items-start">
                ...
                <p className="text-sm text-slate-500 leading-relaxed">
                  Beschreibe den Inhalt deiner Sendung. Mit 100% Community
                  Trust.
                </p>
              </div>
              <div className="flex flex-col items-center md:items-start">
                ...
                <p className="text-sm text-slate-500 leading-relaxed">
                  Digitaler Handshake und Tracking für maximale Sicherheit bei
                  der Übergabe.
                </p>
              </div>
            </div>
          ...
          <div className="bg-white w-full max-w-md rounded-2xl p-6 relative animate-in zoom-in-95">
            ...
            <div className="space-y-3">
              ...
              <textarea
                ...
                className="border p-3 rounded-lg w-full h-24 text-sm"
              />
              <button
                onClick={submitOffer}
                className="w-full bg-slate-900 text-white font-bold py-3 rounded-lg mt-2"
              >
                Veröffentlichen
              </button>
            </div>
          ...
          <div className="bg-white w-full max-w-md rounded-2xl p-6 relative animate-in fade-in zoom-in">
            ...
            <div className="space-y-3">
              ...
              <button
                ...
                className="w-full bg-blue-600 text-white font-bold py-3 rounded-lg mt-2 hover:bg-blue-700"
              >
                Erstellen & Anfrage senden
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white w-full sm:max-w-md h-[90vh] sm:h-[600px] sm:rounded-2xl rounded-t-2xl shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-10">
            <div className="bg-slate-900 text-white p-4 flex justify-between items-center shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-sm">{chatPartnerName}</h3>
            ...
            <div className="flex-1 bg-slate-50 p-4 overflow-y-auto space-y-3">
              {chatMessages.map((msg) => (
                <div
                  key={msg.id}
                  className={`max-w-[85%] p-3 rounded-xl text-sm shadow-sm ${msg.sender_id === currentUserId ? "bg-blue-600 text-white ml-auto rounded-tr-none" : "bg-white border border-slate-200 mr-auto rounded-tl-none"}`}
                >
                  {msg.type === "booking_request" ? (
                    ...
                    >
                      <div className="flex items-center gap-2 font-bold mb-2 border-b border-white/20 pb-1 uppercase text-xs tracking-wider">
                      ...
                      {(() => {
                        ...
                        if (
                          ...
                          console.warn(
                            "DEBUG: Falsches Shipment im Chat angezeigt:",
                            {
                              msgId: msg.id,
                              shipmentTripId: shipment.trip_id,
                              contextTripId: contextTripId,
                        ...
                        return (
                          <>
                            <div className="flex gap-3 items-center">
                              <div className="bg-white/20 w-10 h-10 rounded flex items-center justify-center font-bold text-lg">
                                📦
                              </div>
                              <div>
                                <div className="font-bold text-base">
                                  {shipment.content_desc}
                                </div>
                            ...
                            {(() => {
                                ...
                                return (
                                  <div className="mt-3 p-3 bg-green-100 border-2 border-green-500 rounded-lg text-center">
                                    ...
                                    <p className="text-xs text-green-600">
                                      Das Paket wurde erfolgreich an den
                                      Empfänger übergeben.
                                    </p>
                                  </div>
                                );
                              ...
                              const trip = Array.isArray(shipment.trips)
                                ...
                                : shipment.trips;
                              const isTripOwner =
                                trip && trip.user_id === currentUserId;
                              const isNotMyMessage =
                                msg.sender_id !== currentUserId;
                                        ...
                                        <div className="flex items-start gap-2">
                                          ...
                                          <div className="flex-1">
                                            ...
                                            <p className="text-[10px] text-red-600">
                                              Dieses Paket ({shipment.weight_kg}{" "}
                                              kg) überschreitet deine freie
                                              Kapazität (
                                              {capacityWarning.remaining} kg).
                                            </p>
                                          </div>
                                        </div>
                                      </div>
            ...
            <div className="p-3 bg-white border-t border-slate-200 shrink-0 relative">
              {showBookingOptions && (
                <div className="absolute bottom-full left-0 w-full bg-white border-t border-slate-200 shadow-xl p-4 rounded-t-2xl animate-in slide-in-from-bottom-2 z-20">
                  <div className="flex justify-between mb-3">
                    ...
                    <button onClick={() => setShowBookingOptions(false)}>
                      <X size={16} />
                    </button>
                  </div>
                  <div className="space-y-2 max-h-60 overflow-y-auto">
                    <button
                      ...
                      className="w-full p-3 border-2 border-dashed border-blue-300 bg-blue-50 rounded-lg text-blue-600 font-bold text-sm hover:bg-blue-100 flex items-center justify-center gap-2 transition"
                    >
                      <Plus size={16} /> Neues Paket erstellen
                    </button>
                    {myShipments.map((s) => (
                      <div
                        key={s.id}
                        onClick={async () => {
                          ...
                          if (contextTripId && !s.trip_id) {
                            await supabase
                              ...
                              .eq("id", s.id);
                            console.log(
                              "DEBUG: Shipment trip_id aktualisiert:",
                              { shipmentId: s.id, trip_id: contextTripId },
                            );
                          }
                          sendMessage(
                            "Anfrage senden...",
                      ...
                      >
                        <span className="font-bold text-sm text-slate-700 group-hover:text-blue-600">
                          {s.content_desc}
                        </span>
              ...
              <div className="flex gap-2 items-center pb-safe">
                <button
                  ...
                  className={`p-3 rounded-full transition ${showBookingOptions ? "bg-blue-100 text-blue-600" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}
                >
                  <Plus size={20} />
                </button>
                <input
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/80 p-4">
          <div className="bg-white p-6 rounded-2xl w-full max-w-sm animate-in zoom-in-95">
            <h3 className="font-bold mb-4 text-red-600 flex items-center gap-2">
              <Flag size={18} /> Nutzer melden
            </h3>
            <textarea
              value={reportReason}
              onChange={(e) => setReportReason(e.target.value)}
              className="w-full border p-3 rounded-lg h-24 text-sm mb-4 resize-none focus:ring-2 focus:ring-red-500 outline-none"
              placeholder="Grund..."
            />
            <button
            ...
            <button
              ...
              className="w-full text-slate-400 text-xs mt-3"
            >
              Abbrechen
            </button>
          </div>
